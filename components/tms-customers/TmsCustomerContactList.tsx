...
import type { TmsCustomerContact } from "@/lib/types";
import DeleteButton from "@/components/DeleteButton";

export default function TmsCustomerContactList({
  contacts,
  createContact,
  updateContact,
  deleteContact,
}: {
  contacts: TmsCustomerContact[];
    ...
    <div className="panel p-5">
      ...
      {adding && (
        <form
          ...
          className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5 border border-manifest-line rounded-md p-4 bg-manifest-navy-50/30"
        >
          <ContactFields />
      ...
      <div className="space-y-3">
        {contacts.length === 0 && !adding && <p className="text-sm text-manifest-navy-400">No contacts yet.</p>}
          ...
          editingId === contact.id ? (
            ...
            <div key={contact.id} className="flex items-start justify-between gap-4 border border-manifest-line rounded-md p-3.5">
              <div className="min-w-0">
                <div className="font-medium text-manifest-navy-800">{contact.name}</div>
...
function ContactFields({ contact }: { contact?: TmsCustomerContact }) {
  return (
    <>
      <div>
