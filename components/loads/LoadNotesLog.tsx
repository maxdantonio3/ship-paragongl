...
import type { LoadNote } from "@/lib/types";
import DeleteButton from "@/components/DeleteButton";

export default function LoadNotesLog({
  notes,
  createNote,
  deleteNote,
}: {
  ...
  return (
    <div className="panel p-5">
      <h2 className="font-display text-lg font-medium text-manifest-navy-800 mb-4">
        Quick Notes <span className="text-manifest-navy-400 font-body text-sm font-normal">({notes.length})</span>
      </h2>
      ...
      <form
        action={async (fd) => {
          ...
          await createNote(fd);
          setPending(false);
          const el = document.getElementById("load-note-content") as HTMLTextAreaElement | null;
          if (el) el.value = "";
        }}
        className="mb-5"
      >
        <textarea id="load-note-content" name="content" rows={3} required placeholder="Add a note…" className="field-input" />
