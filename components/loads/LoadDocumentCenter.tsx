...
import { LOAD_DOCUMENT_TYPES, type LoadDocument, type LoadDocumentType } from "@/lib/types";
import { uploadLoadDocument, getLoadDocumentUrl, deleteLoadDocument } from "@/actions/load-documents";
import DeleteButton from "@/components/DeleteButton";

const SINGLE_SLOT_TYPES = LOAD_DOCUMENT_TYPES.filter((t) => t !== "Other");

export default function LoadDocumentCenter({
  loadId,
  documents,
  uploaderEmails,
}: {
  ...
  const currentByType = new Map<LoadDocumentType, LoadDocument>();
  ...
  function handleFileChange(key: string, type: LoadDocumentType, file: File | undefined, description?: string) {
    if (!file) return;
    setError(null);
    setUploadingKey(key);
  ...
  async function handleView(doc: LoadDocument) {
    const url = await getLoadDocumentUrl(doc.file_path);
    if (url) window.open(url, "_blank", "noopener,noreferrer");
    else setError("Couldn't generate a link for this file.");
  }

  async function handleDownload(doc: LoadDocument) {
    const url = await getLoadDocumentUrl(doc.file_path, true);
    if (url) window.open(url, "_blank", "noopener,noreferrer");
              ...
              <div className="flex items-center gap-3 mt-2.5 flex-wrap">
                ...
                {doc && (
                  <>
                    ...
                    <DeleteButton
                      action={() => deleteLoadDocument(loadId, doc.id, doc.file_path)}
